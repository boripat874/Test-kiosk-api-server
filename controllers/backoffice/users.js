...
exports.userscreate = async (req, res) => {
    ...
    const usersaddLogic = new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            //ขอ Portal ID
            // await axios.get(`https://${process.env.CISCO_IP}:${process.env.CISCO_PORT}/ers/config/sponsorportal`,{}, AuthCisco)
            // .then( async(response) => {
                ...
                // ส่งคำขอสร้าง User
                await axios.post(`https://${process.env.CISCO_IP}:${process.env.CISCO_PORT}/ers/config/guestuser`,
                    CiscoUserBody,
                    AuthCisco
                )
                .then((response) => {
                ...
                // ดึง routerid
                await axios.get(
                    `https://${process.env.CISCO_IP}:${process.env.CISCO_PORT}/ers/config/guestuser/name/${Username}`,
        ...
        try {
            ...
            .then( async(rows) => {
               ...
               const CiscoUserBody = {
                ...
                await axios.put(`https://${process.env.CISCO_IP}:${process.env.CISCO_PORT}/ers/config/guestuser/${rows[0].routerid}`, CiscoUserBody,AuthCisco)
                .then((response) => {
                    // console.log(response.data);
                })
                .catch((error) => {
    ...
    const usersdeleteLogic = new Promise(async (resolve, reject) => {
        try {
            ...
            .then( async(rows) => {
                ...
                await axios.delete(`https://${process.env.CISCO_IP}:${process.env.CISCO_PORT}/ers/config/guestuser/${rows.routerid}`,AuthCisco)
                .then((response) => {
                    // console.log(response.data);
                })
                .catch((error) => {
